...
class DyttSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.dytt8.net/html/gndy/dyzz/20181102/57714.html"]

    # def parse(self, response):
    #     """分析首页"""
    #     # 最新电影
    #     latest_moive_link = response.xpath('//*[@id="menu"]/div/ul/li[1]/a')

    def parse_latest_movie(self, response):
        """抓取最新电影列表"""
        # 最新电影列表
        movie_list = response.xpath(
            '//*[@id="header"]/div/div[3]/div[3]/div[2]/div[2]/div[2]/ul')
    ...
    def parse(self, response):        
        ...
        movie["translation"] = [s.strip() for s in movie["translation"].split("/")]
        movie["title"] = [s.strip() for s in movie["title"].split("/")]
        movie["origin_area"] = [s.strip() for s in movie["origin_area"].split("/")]
        ...
        movie["tags"] = [s.strip() for s in movie["tags"].split("|")] if "tags" in movie else []
        movie["actors"] = [movie["actors"]]

        # 没有特殊标记的纯文本
        pure_texts = {s.strip() for s in texts if ("◎" not in s and s.strip())}
        # 获奖的文本
        prize_texts = {s for s in pure_texts if re.search(r"百想艺术大赏|青龙奖|电影大奖|技术奖|金狮奖|电影节|金马|金鹿奖|长春电影节|电影奖", s)}
        print(prize_texts)
        # 剩余的演员
        ...
        # 简介
        movie["Introduction"] = "\n".join([s.strip() for s in set.difference(pure_texts,prize_texts,set(movie["actors"]))])
        # 获奖
